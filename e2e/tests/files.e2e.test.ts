...
import { MOCKED_USER_ID } from "../../src/middleware/__mocks__/auth";
...
describe("/v1 endpoints", () => {
	...
	it("retrieves an existing file", async () => {
		// Arrange
		await request(server).post(baseUrl).attach("file", sampleFile).expect(201);

		// Act
		await request(server).get(baseUrl).expect(200);
	...
	it("replaces a file", async () => {
		...
		// Act
		await request(server).post(baseUrl).attach("file", sample2File).expect(201);
	...
	it("deletes a file", async () => {
		...
		// Act
		await request(server).delete(baseUrl).expect(204);

		// Assert
		await request(server).get(baseUrl).expect(404);
		const filePath = path.join(tempDirectory, MOCKED_USER_ID, clientId, fileId);
	...
	it("deletes all files of a client", async () => {
		...
		await request(server)
			...
			.expect(201);
		await request(server).get(firstFileUrl).expect(200);
		await request(server)
			.post(secondFileUrl)
			.attach("file", sampleFile)
			.expect(201);
		await request(server).get(secondFileUrl).expect(200);

		// Act
		await request(server).delete(url).expect(204);

		// Assert
		await request(server).get(firstFileUrl).expect(404);
		await request(server).get(secondFileUrl).expect(404);
	});
	...
	it("returns 404 for non-existing files", async () => {
		...
		// Act & Assert
		await request(server).get(baseUrl).expect(404);
		await request(server).delete(baseUrl).expect(404);
	});
});
